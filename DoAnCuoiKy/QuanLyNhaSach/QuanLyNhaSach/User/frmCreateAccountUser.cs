...
namespace QuanLyNhaSach.User
{
    public partial class frmCreateAccountUser : Form
    {
            ...
            else
            {
                if (txtMK.Text.Trim() == txtNhapLai.Text.Trim())
                {
                    try
                    {
                        ...
                        cmd.ExecuteNonQuery();
                        // query KHACHHANG table to get MAKH from newly added sql 
                        ...
                        reader.Close();
                        // use MAKH to add login information to the DANGNHAP table
        ...
        private void txtTenTK_Leave(object sender, EventArgs e)
        {
            ...
            {
                errorProvider1.SetError(ctr, "Tên đăng nhập không được chứa khoảng trắng");
            }
            else if (!KT_KhoaNgoai(txtTenTK.Text.Trim()))
            {
        ...
        private void frmCreateAccountUser_Load(object sender, EventArgs e)
        {
            txtTenKH.Focus();
            txtTenKH.SelectionStart = txtTenKH.Text.Length;
            txtTenKH.SelectionLength = 0;
        }
