...
namespace QuanLyNhaSach.Admin
{
    public partial class frmDetailedInvoice : Form
    {
        ...
        private void frmDetailedInvoice_Load(object sender, EventArgs e)
        {
            ...
            if (KenhMua == "Offline")
            {
                lblDonHang.Text = "Đơn hàng Offline";
            }
            else
            {
                lblDonHang.Text = "Đơn hàng Online";
            }
            ThanhToan_Changed();
        ...
        private void dgvAddBook_Load()
        {
            try
            {
                ...
                if (dgvAddBook.Columns["Chon"] == null)
                {
                    DataGridViewCheckBoxColumn chk = new DataGridViewCheckBoxColumn();
                    chk.Name = "Chon";
                    chk.HeaderText = "Chọn";
                    dgvAddBook.Columns.Add(chk); // Thêm cột "Chọn"
                }
                if (connsql.State == ConnectionState.Closed) { connsql.Open(); }
